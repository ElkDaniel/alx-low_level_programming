 ...
 * @i: iterator
 *
 * Description: This function recursively checks if a given number `n` is a prime number
 ...
 * @n: number to check
 *
 * Description: This function checks if a number `n` is a prime number.
 * If `n` is less than or equal to 1, it returns 0 (as prime numbers are
 * greater than 1). Otherwise, it calls the `actual_prime` function with
 * `n` and `n-1` as arguments.
 *
 * Return: 1 if `n` is prime, 0 if not
 */
